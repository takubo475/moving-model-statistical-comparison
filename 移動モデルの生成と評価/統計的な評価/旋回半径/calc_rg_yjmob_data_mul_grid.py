...
def import_data(l_name,f_name):#SQLからデータを抽出する
    ...
    cur = con.cursor()
    cur.execute('SELECT latitudeE7, longitudeE7 FROM '+str(l_name))
    a=cur.fetchall()
    return a
...
if __name__ == '__main__':
    ...
    for uid in range(0,401):

      x_list=[]
      y_list=[]

      file_name = 'data/data_yjmob_B'+str(uid)+'.db'

      data=import_data("expart",file_name)
      for i in range (len(data)):
        
          pattern1 = r'(-?\d+), (-?\d+)'
          d_text=re.search(pattern1, str(data[i]))
          x_list.append(float(d_text.group(1)))
          y_list.append(float(d_text.group(2)))
      ...
      # 格納
      trace_list = []
      trace_list.append( (round((float(trajectory_x[0])), effective_digits2), round((float(trajectory_y[0])), effective_digits2) )) 
      for i in range(1,len(trajectory_x)-1):
          if ( trajectory_x[i-1]!=trajectory_x[i] ) and ( trajectory_y[i-1]!=trajectory_y[i] ):
              trace_list.append( (round((float(trajectory_x[i])), effective_digits2), round((float(trajectory_y[i])), effective_digits2) ))   
      ...
      f = open('result/rg_data_grid/rg_yjmob_uid'+str(uid)+'_data.txt', 'w')
      f.write("uid:"+str(uid))
      f.write('\n'+"R_cm = "+str(r_cm))
      f.write('\n'+"R_g = "+str(R_g))
      f.write('\n'+"R_g2 = "+str(R_g2))
      ...
      f = open('result/rg_data_grid/freq_yjmob_uid'+str(uid)+'_data.txt', 'w')
      f.write("uid:"+str(uid))
      f.write('\n'+"学習前の訪問頻度1番目から10番目:")
