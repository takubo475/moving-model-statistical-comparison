...
if __name__ == '__main__':
    ...
    S=0.0
    for i in range (len(S_rand_list1)):
        S=abs(float(S_rand_list1[i])-float(S_rand_list2[i]))
        S_rand_list.append(S)

    for i in range (len(S_unc_list1)):
        S=abs(float(S_unc_list1[i])-float(S_unc_list2[i]))
        S_unc_list.append(S)

    for i in range (len(S_real_list1)):
        S=abs(float(S_real_list1[i])-float(S_real_list2[i]))
        S_real_list.append(S)
    ...
    sns.set()
    sns.distplot(S_real_list, bins=30)
    sns.distplot(S_unc_list, bins=30)
    sns.distplot(S_rand_list, bins=30)
    plt.legend(labels=["Sreal", "Sunc", "Srand"])
    plt.xlabel("Entropy difference",fontsize=14, color="black")
    plt.ylabel("P(S)",fontsize=14, color="black")
    plt.title("Entropy difference distribution",fontsize=20, color="black")
    plt.tight_layout()
    plt.savefig('result/_graph/time_mesh_difference_yjmob_uid'+str(id_start)+'_'+str(id_end)+'_irl200.png')
    plt.clf()
    plt.close()
